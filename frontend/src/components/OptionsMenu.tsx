...
export default function OptionsMenu({ file, onUpdate }: OptionsMenuProps) {
    ...
    const handleRename = async (file: AppFile) => {
        ...
        if (!error) {
            ...
            try {
                const response = await axios.patch(`${import.meta.env.API_URL}/api/file/rename`, { fileId, newFileName });
...
const handleDelete = async (file: AppFile) => {
    ...
    if (!error) {
        try {
            const response = await axios.delete(`${import.meta.env.API_URL}/api/file/delete`, { data: { fileId, userId } });
            showSuccessToast(response.data.message);
        } catch(error) {
            if (axios.isAxiosError(error)) {
                const message = error.response?.data?.error || "Something went wrong. Please, try again.";
